...
class Password:
    ...
    def __init__(self, digits, char, pwd):
        
        self.digits=digits
        self.char=char[0]
        self.pwd=pwd
        l=digits.split('-')
        self.minDigit=int(l[0])
        self.maxDigit=int(l[1])
    ...
    def validatePassword(self):
        ...
        for x in self.pwd:
            if x==self.char:
                count+=1
        if count>=self.minDigit and count<=self.maxDigit:
            return True
        else:
            return False
    
    def positionPassword(self):
        c1=self.pwd[self.minDigit-1]
        c2=self.pwd[self.maxDigit-1]
        if c1==self.char and c2==self.char:
            return False
        elif c1!=self.char and c2!=self.char:
            return False
        else:
            return True
...
for p in passwords:
    j=p.split(' ')
    n=Password(j[0],j[1],j[2])
    if n.validatePassword():
        validPasswords+=1
