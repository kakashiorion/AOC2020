...
def checkValidity(d):
    ...
    if d['hcl'][0]!='#':
        ...
        for x in d['hcl'][1:]:
            if x.isnumeric()==False and x!='a' and x!='b' and x!='c' and x!='d' and x!='e' and x!='f':
                return False
    if d['ecl']!='amb' and d['ecl']!='blu' and d['ecl']!='brn' and d['ecl']!='gry' and d['ecl']!='grn' and d['ecl']!='hzl' and d['ecl']!='oth':
        return False
    if d['pid'].isnumeric()==False or len(d['pid'])!=9:
        return False
    return True
...
for l in passports:
    if l=='':
        # print(d)
        total+=1
        if len(d)==8 and checkValidity(d):
            validPassports+=1
        elif len(d)==7 and 'cid' not in d.keys() and checkValidity(d):
            validPassports+=1
        d={}
        continue
    else:
