food=['pnglkx nfnzx tjsdp jkbqk rpmqq gzgvdh rgdx szsbj xjdhk zfml ddbmq thvm mvnqdh gsgmdn dtlhh rqqfnlc bxv nthhxn hnmjfl ckqq qrsczjv fkh hkxcb rpcdfph flhfddq qspfqb zmb rpmmv zrgzf jfqqgtl xxfgvz kltcm xjrpr vnfmc xhmmt zkzdrn jmdg xgbvk ngqh hlmvqh djpsmd bnzq rbvdt tfmgl pjln (contains sesame, nuts)',
...
for f in food:
    ...
    ingred=parts[0].split(' ')
    allerg=parts[1][:len(parts[1])-1].split(', ')
    d[k]=[ingred,allerg]
    k+=1
    for i in ingred:
...
for a in allergens:
    ...
    for item in d:
        if a in d[item][1]:
            possibleFoods.append(d[item][0])
    c=[]
    firstFood=possibleFoods[0]
    for x in firstFood:
        p={}
        for y in possibleFoods:
            if x in y:
                if x in p:
                    p[x]+=1
                else:
                    p[x]=1
        if p[x]==len(possibleFoods):
            c.append(x)
    possibleIngredients[a]=c

print(possibleIngredients)

noAllergens=ingredients.copy()
for x in possibleIngredients:
    for y in possibleIngredients[x]:
        if y in noAllergens:
            noAllergens.remove(y)

print(noAllergens,len(noAllergens))
count=0
for item in d:
    for y in d[item][0]:
        if y in noAllergens:
...
considered=[]
for x in range(8):
    for a in possibleIngredients:
        if len(possibleIngredients[a])==1 and a not in considered:
            considered.append(a)
            i=possibleIngredients[a][0]
            for y in possibleIngredients:
                if i in possibleIngredients[y] and len(possibleIngredients[y])>1 and y not in considered:
                    possibleIngredients[y].remove(i)

print(possibleIngredients)
#part2:thvm,jmdg,qrsczjv,hlmvqh,zmb,mrfxh,ckqq,zrgzf
