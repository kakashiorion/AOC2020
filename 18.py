homework=['6 * ((4 * 8 + 4) + (3 * 5 + 3 + 3 * 7 * 5)) + 7 + (9 * (6 + 9 * 7 + 2 * 6 * 6)) + 2',
...
for y in range(len(homework)):
    ...
    for x in homework[y]:
        if x in digits:
            ...
            c=1
            while(res[len(res)-c]!=openBracket):
                c+=1
            res.pop(len(res)-c)
        if len(res)>1 and res[len(res)-1]!=openBracket and res[len(res)-2]!=openBracket:
                c1=res.pop()
                c2=res.pop()
                d=op.pop()
                if d=='+':
...
for y in range(len(homework)):
    ...
    for x in homework[y]:
        if x in digits:
            res.append(int(x))
            if len(res)>2 and res[len(res)-3]!=openBracket and res[len(res)-2]=='+':
                c1=res.pop()
                res.pop()
                c2=res.pop()
                c1=c1+c2
            ...
            while(len(res)>2 and res[len(res)-2]!=openBracket):
                c1=res.pop()
                d=res.pop()
                c2=res.pop()
                if d=='*':
                ...
                res.append(c1)
            res.pop(len(res)-2)
            while(len(res)>2 and res[len(res)-2] =='+'):
                c1=res.pop()
                d=res.pop()
                c2=res.pop()
                c1=c1+c2
