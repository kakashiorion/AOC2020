rules=['vibrant aqua bags contain 1 shiny magenta bag, 2 muted teal bags, 1 dim magenta bag, 1 muted chartreuse bag.',
...
for x in rules:
    ...
    if y[1][:2]=='no':
        ...
        for bag in z:
            ...
            color=j[1]+' '+j[2]
            inner.append(color)
            bags.append((color,int(j[0])))
    d[outer]=inner
    f[outer]=bags

e={}
def countColors(bagColor,d):
    global e
    for color in d:
        if bagColor in d[color]:
            e[color]=1
            countColors(color,d)
        
countColors('shiny gold',d)
print(len(e))
#part1:124

count=0
def countBags(color,f):
    if f[color]==['']:
        print('None')
        return 0
    else:
        s=0
        for x in f[color]:
            s+= x[1]+x[1]*countBags(x[0],f)
        return s

count=countBags('shiny gold',f)
print(count)

# g={}
# for x in f['shiny gold']:
#     g[x[0]]=x[1]
